...
void Graph::shortestPath(string newStart, string newEnd) {
  ...
  for(int i = 0; i < size; i++) { //Copy over vertices to path
...
void Graph::removeNode(string label) {
  ...
  node* startNode;
  edge* collateral;
  for(int i = 0; i < size; i++) { //Remove all connecting edges
  ...
  for(int i = target->id; i < size - 1; i++) { //Shift nodeArray up
    nodeArray[i + 1]->id = nodeArray[i]->id;
    nodeArray[i] = nodeArray[i + 1];
  }
  nodeArray[size] = nullptr;
